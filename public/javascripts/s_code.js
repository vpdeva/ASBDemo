...
// Set last update date YYYYMMDD
var s_code_version = "20160926";

if (location.hostname == "www.asb.co.nz") {
    var s_account = "asb-web-prd";
} else {
    var s_account = "asb-web-dev";
}
...
s.visitorNamespace = "asb";
// s.trackingServer = "visit.asb.co.nz";
// s.trackingServerSecure = "visit.asb.co.nz";
...
s.trackInlineStats = false; // set to false to disable clickmap, true to re-enable
s.linkDownloadFileTypes = "exe,zip,wav,mp3,mov,mpg,avi,wmv,pdf,doc,docx,xls,xlsx,ppt,pptx";
s.linkInternalFilters = "javascript:,www.asb.co.nz,online.asb.co.nz,qa.asb.co.nz,qaonline.asb.co.nz,content.asb.co.nz";
...
s.siteID = "asb"; 			// leftmost value in pagename
...
function s_doPlugins(s) {
    ...
    /* Start global on page variables */

    // set pageName
    if (!s.pageType && !s.pageName) s.pageName = getRealPageName();

    // clean pageName
    if (s.pageName) {
        if (!(s.pageName.indexOf("#") > 0) && !(s.pageName.indexOf("-popup") > 0)) { // if not a modal popup
            s.pageName = s.pageName.replace(RegExp(/\-/g), "");
            s.pageName = s.pageName.replace(RegExp(/\.html/g), "");
        }
        // fix for modal CTA window name not clearing
        if (typeof (s.prop75) != 'undefined' && s.prop75 && s.pageName == s.prop75 && s.pageName.indexOf("#") > 0 && s.pageName.indexOf("popup") > 0) {
            s.pageName = s.pageName.split('#')[0];
        }
    }

    // set content heirarchy variables based on pageName
    if (s.pageName) {
        ...
        s_subsection = s_pagename_array.slice(0, 3).join(':'); // sub section
        // set props/eVars/s.channel
        s.channel = s_section;
        s.eVar7 = s.channel;
        s.eVar2 = s.prop2 = getPageChannelAndSubsectionName();
        s.prop3 = s.eVar3 = window.location.toString();
        s.prop16 = s.eVar16 = s.pageName;
        s.events = s.apl(s.events, "event1", ",", 1);
    }

    var cookies = getCookieInfo();
    // sasmid
    if (!s.eVar50 || s.eVar50 == '') { s.eVar50 = s.prop50 = cookies.sasmid }
    // hcif
    if (!s.eVar1 || s.eVar1 == '') { s.eVar1 = s.prop1 = cookies.hcif }
    // blade list
    if (!s.eVar15 || s.eVar15 == '') { s.eVar15 = s.prop15 = getSectionNames() }
    if (!s.prop5 || s.prop5 == '') { s.prop5 = s.eVar5 = getPageTemplateName() }
    if (!s.prop6 || s.prop6 == '') { s.prop6 = s.eVar6 = getPageCmsUniqueId() }

    //scroll plugin
    s.prop75 = s.eVar69 = s.getPreviousValue(s.pageName, 's_ppn'); // prop75, eVar69: prev page name
    var ppv = s.getPercentPageViewed(s.pageName); // get array of data on prev page % viewed
    if (ppv && typeof ppv == 'object' && ppv[0] == s.prop75) { // if ppv array returned and prev page id matches prev page name
        s.prop70 = ppv[1] + '|' + ppv[2]; // prop70: prev page max and initial % viewed, delimited by "|". (Use classifications to separate values)
        s.prop71 = ppv[4] + 'x' + ppv[5]; // prop71: viewport width x height in CSS pixels
        s.prop72 = ppv[6] + 'x' + ppv[7]; // prop72: display width x height in real pixels
        s.prop73 = ppv[8]; // prop73: pixel density, i.e. number of virtual pixels per real pixel
        s.prop74 = ppv[9]; // prop74: device orientation: initial and final (final only present if rotation occurred)
    }

    // set debug prop & eVar to identify changes and live versions of code
    if ((s_code_version) && (s.version)) {
        s.prop68 = s.version + ":" + s_code_version;
        s.eVar68 = "D=c68";
    } else {
        s.prop68 = "version not set:" + s_code_version;
        s.eVar68 = "D=c68";
    }

    /* End global on page variables */

    /* Start homepage search */

    // capture displayed homepage state {known|unknown}
    if ((!s.prop59) && (s_hpState() != "")) {
        s.prop59 = s.eVar13 = s_hpState();
        s.events = s.apl(s.events, "event102", ",", 1);
    }

    // capture theme displayed to visitor as "theme name"
    if ((!s.prop60) && (s_hpTheme() != "")) {
        s.prop60 = s.eVar14 = s_hpTheme();
        s.events = s.apl(s.events, "event103", ",", 1);
    }

    // set finding method
    // "?fm=internal-search" or "?fm=top-navigation" or "?fm=footer"
    // check to make sure this is not the login function via presence of event104
    if ((s.events) && (s.events.match(/event104/) == null)) {
        s.prop47 = s.getQueryParam("fm");
    }
    s.prop47 = s.prop47.replace(RegExp(/-/g), " ");
    // apply hardcoded value to search start page
    if ((s.pageName) && (s.pageName == "asb:search:start")) {
        s.prop47 = "search";
    }
    if (s.prop47 != "") {
        s.eVar47 = s.prop47;
    ...
    // capture theme/nav/footer link based on query parameter ?link={section:link} on destination page eg.
    // "?link=hptheme1:homeloans" or "?link=footer:homeloans" or "?link=nav:accounts and cards" or "?link=search:search term"
    /* disabled for phase 1
    if (!s.eVar70) {
        s.eVar70 = s.getQueryParam("link");
        if (s.eVar70 != "") {
    ...
    */
    ...
    // capture needs displayed as a comma separated list "need1,need2,need3"
    /* disabled for phase 1
    if ((!s.list1) && (s_needsSelectorStep1DisplayPage() != "")) {
    s.pageName = "asb:needs selector:needs step 1";
    s.list1 = s_needsSelectorStep1DisplayPage().toLowerCase();
    s.events = s.apl(s.events, "event111", ",", 1);
    }
    */

    // append identifiers to urls of modal search pages to differentiate from homepage url
    if ((s.pageName) && (s.pageName == "asb:search:start")) {
        s.prop3 = s.eVar3 = window.location.toString() + "#search-start";
        s.prop2 = s.eVar2 = "asb:search:search";
    }
    if ((s.pageName) && (s.pageName == "asb:search:results")) {
        s.prop3 = s.eVar3 = window.location.toString() + "#search-results";
        s.prop2 = s.eVar2 = "asb:search:search";
    }

    /* End homepage search */

    /* Silverpop integration - message id and recipient id */
    if (!s.campaign) s.campaign = s.eVar64 = s.getQueryParam('et_cid');
    if (!s.eVar65) s.eVar65 = s.getQueryParam('et_rid');
...
function getRealPageName() {
    return ((typeof (_analyticsParams) != 'undefined' && _analyticsParams.pageName && _analyticsParams.pageName != '') ? _analyticsParams.pageName : getFallbackPagename());
}

function getPageCmsUniqueId() {
    return ((typeof (_analyticsParams) != 'undefined' && _analyticsParams.uniqueId && _analyticsParams.uniqueId != '') ? _analyticsParams.uniqueId : '');
}

function getPageTemplateName() {
    return ((typeof (_analyticsParams) != 'undefined' && _analyticsParams.pageTemplate && _analyticsParams.pageTemplate != '') ? _analyticsParams.pageTemplate : '');
}

function getPageChannelAndSubsectionName() {
    return ((typeof (_analyticsParams) != 'undefined' && _analyticsParams.channelPlusSubsection && _analyticsParams.channelPlusSubsection != '') ? _analyticsParams.channelPlusSubsection : '');
}

function getFallbackPagename() {
    var fallbackPagename = s.getPageName();
    if (fallbackPagename) {
        fallbackPagename = fallbackPagename.replace(RegExp(/\-/g), "").replace(RegExp(/\.html/g), "");
        if (!fallbackPagename.indexOf("asb") == 0 && !fallbackPagename.indexOf("qa") == 0) {
            fallbackPagename = "asb:" + fallbackPagename;
        }
    }
    if (fallbackPagename.split(':').length == 2) {
        //missing subsection, probably a sectional home page
        fallbackPagename = fallbackPagename + ':' + fallbackPagename.split(':')[1];
    } else if (fallbackPagename == "asb") {
        // the home page itself
        fallbackPagename = "asb:homepage:homepage";
    }
    ...
    if (document.cookie) {
        ...
        if (ca) {
            for (var i = 0; i < ca.length; i++) {
                var c = ca[i];
                if (c.indexOf("VisitInfo=") > -1) {
                    var hcif = c.replace("VisitInfo=", "");
                    if (hcif && hcif.indexOf("hashedCIF=") > -1) {
                        result.hcif = hcif.replace("hashedCIF=", "").trim();
                    }
                } else if (c.indexOf("mid=") > -1) {
                    result.sasmid = c.replace("mid=", "").trim();
                }
            }
        }
    }
...
function getSectionNames() {
    ...
    var sectionNames = "";
    [].forEach.call(sectionDivs, function (entry) {
        if (entry.hasAttribute("data-blade-title")) {

            //get campaign ids, if the section is campaign
            var isCampaignSection = false;
            var campaignDiv = entry.getElementsByClassName("component-campaign-intro")[0];
            if (typeof campaignDiv != 'undefined') {
                isCampaignSection = true;
                var articleDiv = document.querySelectorAll("article[data-campaign-id]");
                var campaignIds = "";
                [].forEach.call(articleDiv, function (article) {
                    if (article.hasAttribute("data-campaign-id")) {
                        var campId = article.getAttribute("data-campaign-id");
                        if (campaignIds == "") {
                            campaignIds = campId;
                        } else {
                            campaignIds = campaignIds + "-" + campId;
                        }
                    }
                });
            }
            ...
            //get section titles
            var dataVal = entry.getAttribute('data-blade-title');
            if (sectionNames == "") {
                if (isCampaignSection == true) {
                    sectionNames = dataVal + "-" + campaignIds;
                } else {
                    sectionNames = dataVal;
                }
            } else {
                if (isCampaignSection == true) {
                    sectionNames = sectionNames + ":" + dataVal + "-" + campaignIds;
                } else {
                    sectionNames = sectionNames + ":" + dataVal;
                }
            }
        }
    });

    sectionNames = sectionNames.toLowerCase().replace(/[^a-z0-9:-]/gi, '');
    return sectionNames;
}

/* Start homepage search functions */

function determineHpState() {
    var result = "unknown_unrecognised";
    var cookies = getCookieInfo();
    if (cookies) {
        result = ((cookies.hcif && cookies.hcif != "") == true ? "customer" : "unknown") + "_" + ((cookies.sasmid && cookies.sasmid != "") == true ? "recognised" : "unrecognised");
    }
    return result;
}

// return homepage state reflecting if the visitor is known or unknown {known|unknown}
function s_hpState() {
    // ASB to return homepage state based on whether visitor is known or unknown, or ""
    return determineHpState();
}

// return homepage theme reflecting the theme visitor is served
function s_hpTheme() {
    // ASB to return homepage state based on whether visitor is served
    return "{theme name}";
}

// global search function
// searchPage = "{start|results}" eg. "start" is when the modal appears when the field is interacted with, and "results" is when the search results appear
// searchTerm = "search term" as submitted or selected (autocomplete) by the visitor that triggers search results
// searchResults = the number of search results returned as a string ie. "230", if null/no search results return "zero"
// searchSource = the url of the page that the search was started/submitted from, OR "homepage search" TBC
// searchType = the type of search submitted "{typed|search suggest|autocomplete}" based on whether the visitor submitted a typed search term or selected an option from the autocomplete
// searchFilter = the name/s of the filters applied to the search as a comma separated list
function s_hpSearch(searchPage, searchTerm, searchResults, searchSource, searchType, searchFilter) {
    if (searchPage) {
        // clear s object of previous values
        clearVars();
        switch (searchPage.toLowerCase()) {

            // search modal appears
            case "start":
                s.pageName = "asb:search:start";
                s.prop5 = s.eVar5 = "searchpage";
                s.events = s.apl(s.events, "event106", ",", 1);
                break;

                // search results appear
            case "results":
                s.pageName = "asb:search:results";
                s.prop5 = s.eVar5 = "searchpage";
                s.events = s.apl(s.events, "event107", ",", 1);

                // capture search term
                if (searchTerm) {
                    s.prop54 = s.eVar54 = searchTerm.toLowerCase();
                }

                // capture number of results
                if (searchResults) {
                    s.eVar55 = searchResults;
                    if (searchResults == "zero") {
                        // additional null results event
                ...
                // search source
                if (searchSource) {
                    s.eVar56 = searchSource.toLowerCase().split(/[?#]/)[0] + "|";
                } else {
                    s.eVar56 = "no search source|";
                }

                // search type
                if (searchType) {
                    searchType = searchType.toLowerCase();
                    s.eVar56 = s.eVar56 + searchType + "|";
                    // set primary search event
                    if ((searchType == "typed") || (searchType == "autocomplete") || (searchType == "search suggest")) {
                        s.events = s.apl(s.events, "event117", ",", 1);
                    }
                    // set secondary search event
                    if ((searchType == "related query") || (searchType == "show more") || (searchType == "filters") || (searchType == "did you mean")) {
                        s.events = s.apl(s.events, "event118", ",", 1);
                    }
                } else {
                    s.eVar56 = s.eVar56 + "no search type|";
                }

                // search filter
                if (searchFilter) {
                    s.eVar56 = s.eVar56 + searchFilter.toLowerCase();
                } else {
                    s.eVar56 = s.eVar56 + "no search filter";
                }
            ...
            case "back":
                s.events = s.apl(s.events, "event121", ",", 1);
                break;
...
// click from search results function: based on the value passed through will trigger a clicked from a search result, top result, secondary and back click events
// resultType = "result" = click to an internal search result
// resultType = "top" = click to a top internal search result
// resultType = "secondary" = click to an alternate internal search term
function s_internalSearchClick(resultType) {
    if (resultType) {
        s.linkTrackVarsPrev = s.linkTrackVars;
        s.linkTrackEventsPrev = s.linkTrackEvents;
        s.linkTrackVars = "events";
        s.linkTrackEvents = "event126,event127,event128";
        switch (resultType) {
            case "result":
                s.events = "event126";
                break;
            case "top":
                ...
                break;
            case "secondary":
                s.events = "event126,event128";
                break;
            default:
        }
        s.tl(this, 'o', 'internal search click|' + resultType);
        s.linkTrackVars = s.linkTrackVarsPrev;
        s.linkTrackEvents = s.linkTrackEventsPrev;
    }
}

// dynamic page load: capture the needs displayed in step 1 of the needs selector as a comma separated list
// needs = "{need 1,need 2,need 3}" eg. "need1,need2,need3"
function s_needsSelectorStep1DisplayAjax(needs) {
    if (needs) {
        clearVars();
        s.pageName = "asb:needs selector:needs step 1";
        s.prop64 = s.list1 = needs.toLowerCase();
        s.events = s.apl(s.events, "event111", ",", 1);
        s.t();
    }
}
...
// normal page load
function s_needsSelectorStep1DisplayPage() {
    // ASB to return needs displayed as a comma separated list "need1,need2,need3"
    return "";
}

// capture the need clicked in step 1 of the needs selector
// needs = "{need1}" eg. "need1"
function s_needsSelectorStep1Click(needs) {
    if (needs) {
        s.linkTrackVarsPrev = s.linkTrackVars;
        s.linkTrackEventsPrev = s.linkTrackEvents;
        s.list1Prev = s.list1;
        s.linkTrackVars = "prop64,list1,events";
        s.linkTrackEvents = "event112";
        s.events = "event112";
        s.prop64 = s.list1 = needs.toLowerCase();
        s.tl(this, 'o', 'need selector|step 1|need clicked|' + s.list1);
        s.linkTrackVars = s.linkTrackVarsPrev;
        s.linkTrackEvents = s.linkTrackEventsPrev;
        s.list1 = s.list1Prev;
    }
...
// capture the products displayed in step 2 of the needs selector as a comma separated list
// products = "{product 1,product 2,product 3}" eg. "homeloan1,homeloan2,homeloan3"
function s_needsSelectorStep2Display(products) {
    if (products) {
        clearVars();
        s.pageName = "asb:needs selector:products step 2";
        s.list2 = products.toLowerCase();
        s.events = s.apl(s.events, "event113", ",", 1);
        s.t();
    }
}

// capture the product clicked in step 2 of the needs selector
// products = "{product name}" eg. "homeloan"
function s_needSelectorStep2Click(products) {
    if (products) {
        s.linkTrackVarsPrev = s.linkTrackVars;
        s.linkTrackEventsPrev = s.linkTrackEvents;
        s.list2Prev = s.list2;
        s.linkTrackVars = "list2,events";
        s.linkTrackEvents = "event114";
        s.events = "event114";
        s.list2 = products.toLowerCase();
        s.tl(this, 'o', 'need selector|step 2|product clicked|' + s.list2);
        s.linkTrackVars = s.linkTrackVarsPrev;
        s.linkTrackEvents = s.linkTrackEventsPrev;
        s.list2 = s.list2Prev;
    }
...
// set custom menu viewed event each time the nav menu is opened
// products = "{product 1,product 2,product 3}" eg. "homeloan1,homeloan2,homeloan3"
function s_needsSelectorStep2Cancel(products) {
    if (products) {
        s.linkTrackVarsPrev = s.linkTrackVars;
        s.linkTrackEventsPrev = s.linkTrackEvents;
        s.list2Prev = s.list2;
        s.linkTrackVars = "list2,events";
        s.linkTrackEvents = "event115";
        s.events = "event115";
        s.list2 = products.toLowerCase();
        s.tl(this, 'o', 'need selector|step 2|cancel|' + s.list2);
        s.linkTrackVars = s.linkTrackVarsPrev;
        s.linkTrackEvents = s.linkTrackEventsPrev;
        s.list2 = s.list2Prev;
    }
...
// set custom menu viewed event each time the nav menu is opened
// products = "{product 1,product 2,product 3}" eg. "homeloan1,homeloan2,homeloan3"
function s_needsSelectorStep2StartAgain(products) {
    if (products) {
        s.linkTrackVarsPrev = s.linkTrackVars;
        s.linkTrackEventsPrev = s.linkTrackEvents;
        s.list2Prev = s.list2;
        ...
        s.events = "event116";
        s.list2 = products.toLowerCase();
        s.tl(this, 'o', 'need selector|step 2|start again|' + s.list2);
        s.linkTrackVars = s.linkTrackVarsPrev;
        s.linkTrackEvents = s.linkTrackEventsPrev;
        s.list2 = s.list2Prev;
    }
...
// set custom menu viewed event each time the nav menu is opened or closed
// menuAction = "{open|close}" the action being performed
function s_navMenuAction(menuAction) {
    if (menuAction) {
        s.linkTrackVarsPrev = s.linkTrackVars;
        s.linkTrackEventsPrev = s.linkTrackEvents;
        s.linkTrackVars = "prop16,eVar16,events";
        s.linkTrackEvents = "event119,event120";
        mnuAction = menuAction.toLowerCase();
        switch (menuAction) {
            case "open":
                s.events = "event119";
                break;
        ...
        s.tl(this, 'o', 'nav menu:' + menuAction);
        s.linkTrackVars = s.linkTrackVarsPrev;
        s.linkTrackEvents = s.linkTrackEventsPrev;
    }
}

// set custom login click event each time the dropdown login button is clicked
// loginType = "{fnc|ost|mfnc|fnb}" the type of login started
// findingMethod = the finding method location, as per the ?fm equivalent on a page load eg. "asb:header"
function s_loginStart(loginType, findingMethod) {
    if (loginType) {
        return; // temporarily deactivated
        s.linkTrackVarsPrev = s.linkTrackVars;
        s.linkTrackEventsPrev = s.linkTrackEvents;
        s.linkTrackVars = "prop16,prop47,eVar16,eVar47,events";
        s.linkTrackEvents = "event47,event104,event122,event123,event124,event125";
        if (findingMethod) {
            s.eVar47 = s.prop47 = findingMethod;
        } else {
            s.eVar47 = s.prop47 = "asb:header";
        }
        loginType = loginType.toLowerCase();
        switch (loginType) {
            case "fnc":
                s.events = "event47,event104,event122";
                break;
            case "ost":
                s.events = "event47,event104,event123";
                break;
            case "mfnc":
                s.events = "event47,event104,event124";
                break;
            case "fnb":
                s.events = "event47,event104,event125";
                break;
            default:
        }
        s.tl(this, 'o', 'login start:' + loginType);
        s.linkTrackVars = s.linkTrackVarsPrev;
        s.linkTrackEvents = s.linkTrackEventsPrev;
    }
}

// capture notification messages that are displayed
// message = "{message displayed}" eg. "sample message"
function s_notificationDisplay(message) {
    if (message) {
        s.linkTrackVarsPrev = s.linkTrackVars;
        s.linkTrackEventsPrev = s.linkTrackEvents;
        s.linkTrackVars = "prop16,eVar16,eVar66,events";
        s.linkTrackEvents = "event105";
        s.events = "event105";
        s.eVar66 = message.toLowerCase();
        s.tl(this, 'o', 'notification|' + s.eVar66);
        s.linkTrackVars = s.linkTrackVarsPrev;
        s.linkTrackEvents = s.linkTrackEventsPrev;
    }
}

// exit tracking function: concatenate current page name and exit url into a custom link report & custom conv report
// exitUrl = this.href, or whatever you'd like to appear in the reports
function s_exitLink(exitUrl) {
    if (exitUrl) {
        s.linkTrackVarsPrev = s.linkTrackVars;
        s.linkTrackEventsPrev = s.linkTrackEvents;
        s.linkTrackVars = "prop3,prop16,eVar3,eVar16,eVar71,events";
        s.linkTrackEvents = "event71";
        s.events = "event71";
        s.eVar71 = exitUrl + "|" + s.prop16;
        s.tl(this, 'o', 'exit|' + s.eVar71);
        s.linkTrackVars = s.linkTrackVarsPrev;
        s.linkTrackEvents = s.linkTrackEventsPrev;
    }
}

// download tracking function: concatenate current page name and download url into a custom link report & custom conv report
// fileUrl = typically this.href, or whatever you'd like to appear in the reports
function s_downloadLink(downloadUrl) {
    if (downloadUrl) {
        s.linkTrackVarsPrev = s.linkTrackVars;
        s.linkTrackEventsPrev = s.linkTrackEvents;
        s.linkTrackVars = "prop3,prop16,eVar3,eVar16,eVar72,events";
        s.linkTrackEvents = "event72";
        s.events = "event72";
        s.eVar72 = downloadUrl + "|" + s.prop16;
        s.tl(this, 'o', 'download|' + s.eVar72);
        s.linkTrackVars = s.linkTrackVarsPrev;
        s.linkTrackEvents = s.linkTrackEventsPrev;
    }
}

// social tracking function: concatenate page name and social platform into a custom link report & custom conv report
// socialPlatform = social platform name, as you'd like to see it in the reports eg. twitter, facebook, googleplus
function s_socialLink(socialPlatform) {
    if (socialPlatform) {
        s.linkTrackVarsPrev = s.linkTrackVars;
        s.linkTrackEventsPrev = s.linkTrackEvents;
        s.linkTrackVars = "prop3,prop16,eVar3,eVar16,eVar73,events";
        s.linkTrackEvents = "event73";
        s.events = "event73";
        s.eVar73 = socialPlatform + "|" + s.prop16;
        s.tl(this, 'o', 'social|' + s.eVar73);
        s.linkTrackVars = s.linkTrackVarsPrev;
        s.linkTrackEvents = s.linkTrackEventsPrev;
    }
}

/* End homepage search functions */
